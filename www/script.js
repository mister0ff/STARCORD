...
// Admin check
const ADMIN_EMAIL_PREFIX = 'E';
const ADMIN_EMAIL_SUFFIX = '4@gmail.com';
...
function checkAdminStatus() {
  ...
  if (user && user.email) {
    ...
    if (email.startsWith(ADMIN_EMAIL_PREFIX) && email.endsWith(ADMIN_EMAIL_SUFFIX)) {
      isAdmin = true;
...
btnAuthSubmit.addEventListener('click', async () => {
  ...
  if (isLoginMode) {
    if (email.startsWith(ADMIN_EMAIL_PREFIX) && email.endsWith(ADMIN_EMAIL_SUFFIX) && password === ADMIN_PASSWORD) {
      try {
...
async function carregarLoja() {
  ...
  if (itemsSnap.empty) {
    storeItemsContainer.innerHTML = '<div style="color:var(--text-muted);text-align:center;padding:20px;font-size:13px;">Nenhum item disponível.</div>';
    return;
  }
    ...
    if (!isOwned) {
      ...
      btn.addEventListener('click', async () => {
        ...
        if (itemSnap.exists()) {
          ...
          // Verificar se é efeito de texto
          if (itemData.type === 'rainbow' || itemData.type === 'gradient') {
            // Salvar estilo escolhido
            await setDoc(doc(db, "profiles", currentUsername), { 
              textEffect: itemData.type,
              textEffectData: itemData.effectData || {}
            }, { merge: true });
            showToast('Efeito de texto aplicado!', 'success');
          } else {
            await setDoc(doc(db, "profiles", currentUsername), { 
  ...
  if (currentServerId) {
    ...
    if (isMember) {
      ...
      for (const stars of starOptions) {
        ...
        btn.addEventListener('click', async () => {
          await darAstros(username, stars);
        });
...
// ==================== ASTROS ====================
async function darAstros(username, quantidade) {
  ...
  try {
    ...
    await runTransaction(db, async (transaction) => {
      ...
      transaction.update(serverRef, { stars: currentStars + quantidade });
    });
    showToast(`${quantidade} Astros dados a @${username}! ⭐`, 'success');
...
function getLevelBenefits(level) {
  const benefits = {
    1: ['📌 Banner do servidor', '🎨 Cor personalizada'],
    2: ['🖼️ Galeria de imagens', '✨ Ícone GIF suportado'],
    3: ['🌈 Cargos com degrade', '🔤 Fontes diferentes'],
    4: ['🎯 Mais slots de cargos', '📊 Analytics do servidor'],
    5: ['🌟 Emojis personalizados', '🎵 Integração de música'],
...
async function processarTextoComEmbeds(text, bodyDiv, userRoles = []) {
  ...
  // Cores e animações (efeitos comprados)
  const profileSnap = await getDoc(doc(db, "profiles", currentUsername));
...
function setupMessageInteractions(row, msgId, data) {
  ...
  // Swipe para responder (arrastar para direita)
  let touchStartX = 0;
  row.addEventListener('touchstart', (e) => {
  ...
  row.addEventListener('touchend', () => {
    setTimeout(() => { isSwiped = false; }, 300);
  });

  // Clique em reação para adicionar/remover
...
function showReactionTooltip(row, msgId) {
  ...
  reactionTooltip.style.left = `${rect.left + rect.width / 2 - 140}px`;
  reactionTooltip.style.top = `${rect.top - 60}px`;
...
function showReplyTooltip(row, msgId, data) {
  const rect = row.getBoundingClientRect();
  replyTooltip.style.left = `${rect.left + rect.width / 2 - 100}px`;
  replyTooltip.style.top = `${rect.top - 50}px`;
  ...
  try {
    ...
    for (const docSnap of rolesSnap.docs) {
      ...
      div.innerHTML = `
        ...
        ${!role.isDefault ? `<button class="btn starcord-btn" style="padding:2px 8px;font-size:10px;" data-roleid="${roleId}">Editar</button>` : ''}
      `;
